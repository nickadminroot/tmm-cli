...
- **Kinematics:** Obtain the native XMCD with `tmm xmcd MODEL.yaml --output
  ...
  originally generated by Mathcad. Every mechanism calculation also needs a
  matching high-level `calculation-schematic` scene; follow the mandatory
  synchronization rule in [`mathcad-mechanisms`](../mathcad-mechanisms/SKILL.md).
