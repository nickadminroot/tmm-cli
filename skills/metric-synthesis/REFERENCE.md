...
| --- | --- | --- |
| `slider_crank.two_positions_stroke` | `unknowns.l_1`, `unknowns.phi_2H`, `unknowns.phi_2K`, `unknowns.X_CH`; `derived.l_2`, `derived.e` | Use the H pose: ground point A, crank point B at `phi_1H`, rod point C from the solved `l_2` at slider ordinate `e`, and a ground slot for the slider. The K pose and stroke are synthesis constraints, not simultaneous YAML geometry. |
